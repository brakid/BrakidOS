...
#define TAABLE_RECORD_SIZE sizeof(byte)     // 1 byte to keep flags for memory

#define HEAP_END           0x3FFFFFFF // 1024-128MB heap
#define HEAP_START         0x08000000 // 128MB code + kernel stack
#define MEMORY_TABLE_END   ((HEAP_END - HEAP_START) / (TAABLE_RECORD_SIZE + ALLOCATION_SIZE)) // end of memory table
